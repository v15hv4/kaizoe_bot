...
class EpicFilter(BaseFilter):
    def filter(self, message):
        if message.text:
            return message.text.lower() == 'epic' or message.text.lower() == 'epic moment' or message.text.lower() == 'epico'
...
@run_async
def epic(bot: Bot, update: Update):
    message = update.effective_message
    epic_count = sql.new_epic_moment(message.chat.id)

    bot.send_message(
        message.chat.id,
        'An epic moment has been reported.\n\nTotal epic moments in this chat so far: %d`' % (int(epic_count)),
        parse_mode = ParseMode.MARKDOWN
    )
...
EPIC_COUNT_HANDLER = MessageHandler(epic_filter, epic)
...
dispatcher.add_handler(EPIC_COUNT_HANDLER)
